...
class VisDoMRAG:
        ...
        if self.vision_retriever in ["colpali", "colqwen"]:
            if self.vision_retriever == "colpali":
                ...
                try:
                    ...
                    self.vision_processor = ColQwen2Processor.from_pretrained("vidore/colqwen2-v0.1",size={"shortest_edge": 1000, "longest_edge": 1000} )
                except ImportError:
    ...
    def cache_documents(self):
        """
        Cache document content for all PDFs in the dataset with parallel processing.
    ...
    def identify_document_and_page_batch(self, chunks):
        """
        Identify which document and page chunks belong to using vectorized operations.
        
        Args:
            chunks (list): List of text chunks
            
        Returns:
            list: List of (arxiv_id, page_num) tuples
        """
        results = []
        
        # Pre-compute all page texts for faster comparison
        doc_pages = {}
        for arxiv_id, pages in self.document_cache.items():
            doc_pages[arxiv_id] = pages
        
        for chunk in chunks:
            max_ratio = 0
            best_match = (None, None)
            
            # Use a more efficient similarity check for long texts
            chunk_words = set(chunk.lower().split())
            
            for arxiv_id, pages in doc_pages.items():
                for page_num, page_text in enumerate(pages):
                    # Quick word-based similarity check first
                    page_words = set(page_text.lower().split())
                    if len(chunk_words) > 0:
                        word_overlap = len(chunk_words & page_words) / len(chunk_words)
                        
                        # Only do expensive sequence matching if word overlap is promising
                        if word_overlap > 0.3:
    ...
    def build_visual_index(self):
        ...
        try:
            ...
            # Generate query embeddings in batches
            query_embeddings = {}
            batch_queries = []
            batch_q_ids = []
    ...
    def build_text_index(self):
        """
        Build text index for all documents in the dataset with optimizations.
        ...
        try:
            ...
            # Prepare all documents with batch processing
            all_chunks = []
            chunk_to_doc_mapping = []
            
            # Process documents in batches for memory efficiency
            doc_items = list(self.document_cache.items())
            batch_size = max(1, len(doc_items) // self.max_workers)
            
            for i in tqdm(range(0, len(doc_items), batch_size), desc="Processing documents for text index"):
                batch_docs = doc_items[i:i + batch_size]
                
                batch_chunks = []
                batch_mappings = []
                
                for doc_id, pages in batch_docs:
                    all_text = "\n".join(pages)
                    chunks = self.split_text(all_text)
                    batch_chunks.extend(chunks)
                    
                    # Batch process document identification
                    chunk_mappings = self.identify_document_and_page_batch(chunks)
                    
                    for chunk, (arxiv_id, page_num) in zip(chunks, chunk_mappings):
                ...
                chunk_to_doc_mapping.extend(batch_mappings)
            
            logger.info(f"Total chunks created: {len(all_chunks)}")
            
            # Initialize retriever based on selected method
            if self.text_retriever == "bm25":
                # BM25 indexing with batch processing
                ...
                for _, row in self.df.iterrows():
                    queries_batch.append((row['q_id'], row['question']))
                    
                    if len(queries_batch) >= self.batch_size:
                        batch_results = self._process_bm25_batch(
                            queries_batch, bm25_model, all_chunks, chunk_to_doc_mapping
                        )
                ...
                logger.info(f"Building {self.text_retriever.upper()} index...")
                ...
                for i in tqdm(range(0, len(all_chunks), self.batch_size), desc="Adding chunks to index"):
                ...
                for _, row in self.df.iterrows():
                    ...
                    if len(queries_batch) >= self.batch_size:
                        batch_results = self._process_st_batch(
                            queries_batch, collection, all_chunks, chunk_to_doc_mapping
                        )
                        results.extend(batch_results)
                        queries_batch = []
            ...
            # Save results to CSV
            logger.info(f"Saving {len(results)} results to {self.text_retrieval_file}")
    ...
    def _process_bm25_batch(self, queries_batch, bm25_model, all_chunks, chunk_to_doc_mapping):
        """Process a batch of queries with BM25."""
        results = []
        
        for q_id, question in queries_batch:
            try:
                # Get BM25 scores
    ...
    def _process_st_batch(self, queries_batch, collection, all_chunks, chunk_to_doc_mapping):
        """Process a batch of queries with sentence transformers."""
        results = []
        
        # Extract questions for batch query
        questions = [question for _, question in queries_batch]
        
        try:
            # Query collection with all questions at once
            ...
            for i, (q_id, question) in enumerate(queries_batch):
                try:
                    for rank, (chunk_id, score) in enumerate(zip(
                        query_results['ids'][i],
                        query_results['distances'][i]
                    )):
    ...
    def retrieve_visual_contexts(self, query_id):
        ...
        """
        try:
            if not os.path.exists(self.vision_retrieval_file):
                logger.error(f"Visual retrieval file not found: {self.vision_retrieval_file}")
                logger.error("Please build visual index first using build_visual_index() method")
                return []
    ...
    def retrieve_textual_contexts(self, query_id):
        ...
        """
        try:            
            if not os.path.exists(self.text_retrieval_file):
                logger.error(f"Text retrieval file not found: {self.text_retrieval_file}")
                logger.error("Please build text index first using build_text_index() method")
                return []
    ...
    def build_indexes(self):
        """
        Build both visual and text indexes if they don't exist or if force_reindex is True.
        
        Returns:
            tuple: (visual_success, text_success)
        """
        visual_success = True
        text_success = True
        
        # Build visual index if needed
        if not os.path.exists(self.vision_retrieval_file) or self.force_reindex:
            logger.info("Building visual index...")
            visual_success = self.build_visual_index()
            if visual_success:
                logger.info("Visual index built successfully!")
            else:
                logger.error("Failed to build visual index!")
        else:
            logger.info(f"Visual index already exists: {self.vision_retrieval_file}")
        ...
        if not os.path.exists(self.text_retrieval_file) or self.force_reindex:
            ...
            text_success = self.build_text_index()
            if text_success:
                logger.info("Text index built successfully!")
            else:
                logger.error("Failed to build text index!")
        else:
            logger.info(f"Text index already exists: {self.text_retrieval_file}")
        
        return visual_success, text_success

    def run(self, build_indexes_first=True):
        """
        Run the VisDoMRAG pipeline on all queries in the dataset.
        
        Args:
            build_indexes_first (bool): Whether to build indexes before processing queries
        """
        logger.info("Starting VisDoMRAG pipeline")
        
        # Build indexes first if requested
        if build_indexes_first:
            logger.info("Building indexes before processing queries...")
            visual_success, text_success = self.build_indexes()
            
            if not visual_success:
                logger.error("Visual index building failed. Cannot proceed with pipeline.")
                return
            
            if not text_success:
                logger.error("Text index building failed. Cannot proceed with pipeline.")
                return
            
            logger.info("All indexes built successfully. Starting query processing...")
        
        # Process each query
        successful_queries = 0
        failed_queries = 0
        
        for query_id in tqdm(self.df['q_id'].unique(), desc="Processing queries"):
            try:
        ...
        logger.info(f"VisDoMRAG pipeline completed!")
        logger.info(f"Successful queries: {successful_queries}")
        logger.info(f"Failed queries: {failed_queries}")
        logger.info(f"Total queries: {successful_queries + failed_queries}")

    def run_without_building_indexes(self):
        """
        Run the pipeline assuming indexes are already built.
        This is faster for subsequent runs.
        """
        self.run(build_indexes_first=False)
